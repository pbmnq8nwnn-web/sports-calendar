...
賽事資料來源都是外部公開 API，偶爾會 timeout 或暫時掛掉。單純「這次抓不到就當作 0 筆」會被 diff 邏輯誤判成「賽事全部被取消了」，下次 API 恢復又誤判成「全部新增」，一來一往就是兩則假 Telegram 通知（VNL 曾在 2026-08-24 因為官網 API timeout 連續發生兩次，41 場比賽反覆判定成移除/新增）。`generate.py` 有兩層防護：

1. **抓取失敗時沿用舊資料**（`carry_over`）：目前套用在 VNL、F1（整支 API 一次掛掉就整批歸零風險最高的兩個）。某次抓取整批失敗時，把舊 `docs/sports.ics` 裡同運動、這次沒抓到的場次原封不動補回去，下次抓取成功會直接蓋過。
2. **過去賽事永久凍結**（`apply_past_freeze`）：已經開打超過 24 小時的比賽（對戰雙方/時間/地點都已定案，不會再變），一律沿用舊 `docs/sports.ics`，完全不管這次 API 抓到什麼，涵蓋全部五種運動。24 小時緩衝是為了容忍 MLB 雨延延賽（補賽是另一筆賽事，不受影響）；如果官方真的把某場過去比賽改到未來（例如補賽），系統會偵測到新資料的時間已經在未來，照樣正常反映異動。

另外，diff 結果全空（新增/移除/異動/改期都是 0）時，`generate.py` 連 `docs/sports.ics` 都不會覆寫，從根本上保證「內容沒變就不 commit、不通知」。
...
- **失敗保護**：API 抓取失敗只記警告、不中斷整個 workflow（NBA/MLB/F1 等其他賽事照常執行），也不會把「這次沒抓到」誤判成賽事取消，詳見上方「資料穩定性設計」
